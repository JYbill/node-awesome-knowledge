...
- **使用索引的代价**：修改索引字段的写操作（插入、更新和删除）会花费更长的时间，但是对于读多写少的场景这样的代价是完全可以接受的如果这是一个很少用到的查询或者是由管理员执行的不太关心时间消耗的查询，那么就不应该在此上面创建索引。
...
"executionStats": {
	"nReturned": 1, // 返回一条文档
	"executionTimeMillis": 584, // 耗时584ms
	"totalDocsExamined": 1000000, // 扫描文档总数10w条
	"executionStages": {
		"stage": "COLLSCAN" // 文档扫描
	}
}
...
"executionStats": {
	"nReturned": 1, // 返回一条文档
	"executionTimeMillis": 1, // 耗时1ms
	"totalDocsExamined": 1, // 扫描文档总数1条
	"executionStages": {
		"stage": "FETCH" // 查找索引
	}
}
...
// 复合索引下的文档数据，先按age字段升序，再按username升序
...
> ⚠️ 可以看到添加索引后，会对索引进行排序，所以排序方向并不重要：MongoDB 可以在任意方向上遍历索引

- 等值查询并排序

```ts
db.users.find({ age: 21 }).sort({ username: -1 });
// 先找到age === 21，再根据文档降序获取，全部都走的索引
// MongoDB可以直接跳转到正确的年龄，并且不需要对结果进行排序，因为只要遍历索引就会以正确的顺序返回数据
```
...
// 我们可以发现21~30岁的年龄文档数据获取到之后，username需要在内存中重新排序，因为我们之前的索引是先排序age再排序username
// 解决方案1：修改复合索引	{username: 1, age: 1}
// 解决方案2：分段获取少量文档

// 使用 {username: 1, age: 1}后会扫描索引，但是不用在内存中排序
...
[user10000, 31]
[user100000, 21]
...
db.users.find({"age" : {"$gte" : 21, "$lte" : 30}}).sort({"username" : 1})
// 逐条索引筛选出[21, 30]的数据，而结果则是已经排序好的内容
```

### 5.1.3 　 MongoDB 如何选择索引

- 假设有一个查询进入，5 个索引中的 3 个被标识为该查询的候选索引。然后，MongoDB 会创建 3 个查询计划，分别为每个索引创建 1 个，并在 3 个并行线程中运行此查询，每个线程使用不同的索引。这样做的目的是看哪一个能够最快地返回结果
  ![](./images/5-1.jpg)
- **查询竞赛的意义**：对于具有相同形状的后续查询，MongoDB 知道要选择哪个索引，一个获胜计划存储在缓存中，以备在将来用于进行该形状的查询。随着时间的推移以及集合和索引的变化，查询计划可能会从缓存中被淘汰。MongoDB 会再次进行尝试，以找到最适合当前集合和索引集的查询计划。
- **导致竞赛缓存失败的原因**：重建特定的索引、添加或删除索引，或者显式清除计划缓存。此外，mongod 进程的重启也会导致查询计划缓存丢失。
